...
A package for scraping the user-defined NFL season's schedule/results (`scrape_schedule`), simulate the current week (`simulate_current_week`) and season (`nfl_season_simulation`), and generate postseason probabilities (`nfl_postseason_probabilities`) using the [`game_predictions`](https://github.com/aaronengland/game_predictions/blob/master/README.md) algorithm.
